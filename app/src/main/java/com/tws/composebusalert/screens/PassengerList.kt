...
//                                    showExitDialog.value=true
                ...
                CreateView(passenderList = passengerDetails.value, navController, loginViewModel) {
                    scope.launch {
...
//        contentPadding = PaddingValues(horizontal = 16.dp, vertical = 8.dp)
            ...
            items(items) { item ->
                CardViewStudent(studentList, item, navController, loginViewModel) {
                    onCardClick()
                }
            }
        }
...
fun CardViewStudent(
    studentList: List<PassengerDetailResponse>,
    index: Int,
    navController: NavController, loginViewModel: DriverLoginViewModel,
    onCardClick: () -> Unit
) {
...
//                        loginViewModel.getCurrentBusLocation()
        ...
        if (isSnackbarVisible.value) {
            SnackBars(onDismiss = { isSnackbarVisible.value = false })
        }
